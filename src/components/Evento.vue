<template>
  <div class="event-card" v-if="evento">
    <img :src="evento.imageUrl || 'default-image-url.jpg'" :alt="evento.name || 'No Name Available'"
      class="event-image" />
    ...
    <button @click="setDetalles" class="detail-button">
      Ver Detalles
    </button>
  </div>
  <div v-else>
...
export default {
  ...
  methods: {
    ...
    setDetalles() {
      const eventStore = useEventStore();
      eventStore.vistaDetalles(this.evento); 
...
.event-card:hover .detail-button {
